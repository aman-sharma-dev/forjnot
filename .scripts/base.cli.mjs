...
program
  .version('1.0.0')
  .description('CLI for managing your Stackbase project');
...
const mainMenu = async () => {
  ...
  console.log(chalk.green('2. Start the server: pnpm run server'));
  console.log(chalk.green('3. Generate a new module: pnpm run generate:module'));

  const choices = [
    { name: 'Build the project', value: 'build' },
    { name: 'Start the server', value: 'start' },
...
const generateModule = (moduleName) => {
  ...
  fs.writeFileSync(usersModulePath, `import { Module } from '@nestjs/common';
...
`);
  ...
  fs.writeFileSync(moduleIndexPath, `export * from './${moduleName}.module';\n`);

  console.log(chalk.green(`Module ${moduleName} created successfully at ${baseDir}!`));
};
